...
///     关闭时直接交还原生相机，不做插值。相机位置刻意**不持久化**。///   - **不接管 FOV**（连 `getFov`
///     都不钩）：取景角度完全维持原样。///   - **不限制**离玩家的距离。代价是区块由服务端按**玩家**位置流送，
...
private:
    ...
    std::optional<bool> mSavedHideItemInHand;
};
